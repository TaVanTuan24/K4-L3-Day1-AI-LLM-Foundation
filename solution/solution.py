...
def compare_models(prompt: str) -> dict:
    ...
    # Lấy giá theo model đang dùng (fallback về gpt-4o nếu không có trong bảng)
...
def run_assistant(
    ...
    history, num_turns, total_tokens, total_cost = [], 0, 0, 0.0
    while True:
        # Kiểm tra max_turns TRƯỚC khi đọc input — max_turns=0 phải thoát ngay
        if max_turns is not None and num_turns >= max_turns:
            break
        user_msg = get_input()
        if user_msg.strip().lower() in ("quit", "exit"):
        ...
        # Gọi API qua retry để chịu lỗi tạm thời (429, timeout, ...) — stream=True
        ...
        # In từng chunk khi stream về và ghép lại thành reply hoàn chỉnh
        reply = ""
        for chunk in stream:
            delta = chunk.choices[0].delta.content or ""
            print(delta, end="", flush=True)
        ...
        # Cập nhật history (user + assistant), giữ tối đa 3 lượt cuối (6 message)
        history.append({"role": "user", "content": user_msg})
        history.append({"role": "assistant", "content": reply})
        history = history[-6:]

        # Cộng dồn thống kê bằng count_tokens và estimate_cost
        num_turns += 1
